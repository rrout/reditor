...
function! s:GetTypeInfoOfVariable(contextStack, szVariable, bSearchDecl)
	...
	""call ECHOER("VARIBLE TYPE", a:contextStack, a:szVariable, a:bSearchDecl)
    if a:bSearchDecl
        " Search type of declaration
        "let result = s:SearchTypeInfoOfDecl(a:szVariable)
        let result = s:SearchDecl(a:szVariable)
		""call ECHOER("DECL TYPE", result)
    endif
    "KALS
    if result != {}
        let szFilter = "index(['m', 'v'], v:val.kind[0])>=0"
        let tagItem = s:ResolveSymbol(a:contextStack, result.value, szFilter)
        if tagItem!={}
            let result.value = tagItem.name
            ""call ECHO("TOKEN", tagItem.name, " RESULT :", result)
            return result
        endif
...
function! s:ResolveSymbol(contextStack, szSymbol, szTagFilter)
	""call ECHOER("RESOLVE SYM", a:contextStack, a:szSymbol, a:szTagFilter)
    let tagItem = {}
    for szCurrentContext in a:contextStack
		...
		""call ECHOER("RESOLVE SV", tagList)
		"'index([''m'', ''v''], v:val.kind[0])>=0'
