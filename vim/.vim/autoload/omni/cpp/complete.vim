...
function! omni#cpp#complete#Init()
    ...
    "inoremap <expr>	<C-M> omni#cpp#maycomplete#IncludeStd()
    "inoremap <expr>	<C-J> omni#cpp#maycomplete#Complete()
    "inoremap <expr> " omni#cpp#maycomplete#IncludeUsr()
...
function! s:TagPopupList1(szTypeName, szBase)
    let result = []
...
function! s:TagPopupList(szTypeName, szBase)
	...
	let szTagQuery = '^' . a:szTypeName . '::' . a:szBase . '.*$'

	"echoerr szTagQuery
	"echoerr "LIST TAG"
	" Getting tags
	let result = omni#common#utils#GetGtag('dx', szTagQuery)
	let taglist = omni#common#utils#GetGtagList(result)
	"echon taglist
	"echoerr "TAGLIST POPUP"

	" We extend tag items to popup items
	call map(taglist, 's:ExtendTagItemToPopupItem(v:val, a:szTypeName)')
...
function! s:IncludePopupList(szTypeName, szBase)
    let taglist = []

	let szTagQuery = a:szTypeName

	""call ECHOER("IncludeTag", szTagQuery)
	" Getting tags
	let result = omni#common#utils#GetGtag('P', szTagQuery)
	""call ECHOER("INCLUDE RESULT", result)
	let taglist = omni#common#utils#GetGtagFileList(result)

	""call ECHOER("INCLUDE POPUP", taglist)
...
function! s:InitComplete()
	...
	""call ECHOER("INCLUDE==== +", string(g:omni#cpp#includes#CACHE_INCLUDES))
    " Check for updates
    for szIncludeName in keys(g:omni#cpp#includes#CACHE_INCLUDES)
		""call ECHOER("INCLUDE ", szIncludeName)
...
function! omni#cpp#complete#Main(findstart, base)
    ...
    if a:findstart
        ...
        if !s:bMayComplete
			...
			""call ECHOER("SYMBOL", g:omni#cpp#items#data, " CurrentInst=>", omni#cpp#utils#TokenizeCurrentInstruction())
        endif
    ...
    if len(g:omni#cpp#items#data)==0
		...
		""call ECHOER("FIND CLASS", s:contextStack, g:omni#cpp#items#data)
		if g:omni#cpp#items#data[0].kind == 'include'
			let list = s:IncludePopupList('.*\.h$', 'Px')
			""call ECHOER("INCLUDE FILES", list)
			return list
		endif
        ...
        if typeInfo != {}
            if g:omni#cpp#items#data[-1].kind == 'itemScope'
				...
				""call ECHOER("CLASS MEM", s:contextStack, typeInfo)
                for resolvedTagItem in omni#cpp#utils#GetResolvedTags(s:contextStack, typeInfo)
                    let szTypeInfo = omni#cpp#utils#ExtractTypeInfoFromTag(resolvedTagItem)
                    if index(s:contextStack, szTypeInfo)<0
                        let szAccessFilter = 'public'
					...
					""call ECHOER("MEMBER::", resolvedTagItem, a:base, szAccessFilter)
                    call s:FilterClassMembers(s:SearchMembers(resolvedTagItem, a:base), szAccessFilter)
                endfor
				""call ECHO("RESOLEVED", resolvedTagItem)
            endif
        endif
    endif


	"call ECHO("RESULT", s:popupItemResultList)
